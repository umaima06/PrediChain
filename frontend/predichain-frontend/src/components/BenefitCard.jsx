...
const BenefitCard = ({ icon, title, value, description }) => {
  return (
    ...
    {/* Icon */}
      ...
      {/* Value */}
      {value && <p className="text-2xl font-bold mb-2 text-blue-600 dark:text-teal-300">{value}</p>}

      {/* Description */}
      {description && <p className="text-gray-700 dark:text-gray-200 text-center">{description}</p>}
    </div>
  );
};
